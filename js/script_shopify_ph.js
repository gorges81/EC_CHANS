// /js/script_shopify_ph.js

document.addEventListener('DOMContentLoaded', function() {
    const correlativoSelect = document.getElementById('correlativoSelect');
    const btnIntegrar = document.getElementById('btnIntegrarShopifyPH');
    const btnVaciar = document.getElementById('btnVaciarShopifyPH');
    const btnVerDatos = document.getElementById('btnVerDatosShopifyPH');
    ...
    if (btnIntegrar) {
        btnIntegrar.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                html: `Se borrarán los datos actuales de 'Shopify PH' y se procesarán los del lote <b>${selectedText}</b>.`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    Swal.fire({
                        title: 'Procesando...',
                        html: 'Migrando datos de Shopify PH. Por favor espere.',
                        allowOutsideClick: false,
                        didOpen: () => { Swal.showLoading(); }
                    });

                    $.ajax({
                        url: '/ec_chans/app/integrador/shopify_ph/procesar_shopify_ph.php',
                        type: 'POST',
    ...
    if (btnVaciar) {
        btnVaciar.addEventListener('click', function() {
            Swal.fire({
                title: '¿Estás seguro?',
                text: "¡Todos los registros de Shopify PH serán eliminados permanentemente!",
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    $.post('/ec_chans/app/integrador/shopify_ph/vaciar_shopify_ph.php', function(response) {
                        if(response.success) {
    ...
    if (btnVerDatos) {
        btnVerDatos.addEventListener('click', function() {
            ...
            $.post('/ec_chans/app/integrador/shopify_ph/obtener_datos_grid_shopify_ph.php', { correlativo: correlativo }, 'json')
