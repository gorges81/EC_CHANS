// /js/script_walmart_item.js

document.addEventListener('DOMContentLoaded', function() {
    const correlativoSelect = document.getElementById('correlativoSelect');
    const btnIntegrar = document.getElementById('btnIntegrarWalmartItem');
    const btnVaciar = document.getElementById('btnVaciarWalmartItem');
    const btnVerDatos = document.getElementById('btnVerDatosWalmartItem');
    ...
    if (btnIntegrar) {
        btnIntegrar.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                html: `Se borrarán los datos actuales de 'Walmart Item' y se procesarán los del lote <b>${selectedText}</b>.`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    Swal.fire({
                        title: 'Procesando...',
                        html: 'Migrando datos de Walmart Item. Por favor espere.',
                        allowOutsideClick: false,
                        didOpen: () => { Swal.showLoading(); }
                    });

                    $.ajax({
                        url: '/ec_chans/app/integrador/walmart_item/procesar_walmart_item.php',
                        type: 'POST',
    ...
    if (btnVaciar) {
        btnVaciar.addEventListener('click', function() {
            Swal.fire({
                title: '¿Estás seguro?',
                text: "¡Todos los registros de Walmart Item serán eliminados permanentemente!",
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    $.post('/ec_chans/app/integrador/walmart_item/vaciar_walmart_item.php', function(response) {
                        if(response.success) {
    ...
    if (btnVerDatos) {
        btnVerDatos.addEventListener('click', function() {
            ...
            $.post('/ec_chans/app/integrador/walmart_item/obtener_datos_grid_walmart_item.php', { correlativo: correlativo }, 'json')
