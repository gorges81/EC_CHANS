// /js/script_walmart_inv.js

document.addEventListener('DOMContentLoaded', function() {
    const correlativoSelect = document.getElementById('correlativoSelect');
    const btnIntegrar = document.getElementById('btnIntegrarWalmartInv');
    const btnVaciar = document.getElementById('btnVaciarWalmartInv');
    const btnVerDatos = document.getElementById('btnVerDatosWalmartInv');
    ...
    if (btnIntegrar) {
        btnIntegrar.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                html: `Se borrarán los datos actuales de 'Walmart Inventory' y se procesarán los del lote <b>${selectedText}</b>.`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    Swal.fire({
                        title: 'Procesando...',
                        html: 'Migrando datos de Walmart Inventory. Por favor espere.',
                        allowOutsideClick: false,
                        didOpen: () => { Swal.showLoading(); }
                    });

                    $.ajax({
                        url: '/ec_chans/app/integrador/walmart_inv/procesar_walmart_inv.php',
                        type: 'POST',
    ...
    if (btnVaciar) {
        btnVaciar.addEventListener('click', function() {
            Swal.fire({
                title: '¿Estás seguro?',
                text: "¡Todos los registros de Walmart Inventory serán eliminados permanentemente!",
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    $.post('/ec_chans/app/integrador/walmart_inv/vaciar_walmart_inv.php', function(response) {
                        if(response.success) {
    ...
    if (btnVerDatos) {
        btnVerDatos.addEventListener('click', function() {
            ...
            $.post('/ec_chans/app/integrador/walmart_inv/obtener_datos_grid_walmart_inv.php', { correlativo: correlativo }, 'json')
