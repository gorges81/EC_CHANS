// /js/script_inv_shopify_ppb.js

document.addEventListener('DOMContentLoaded', function() {
    const correlativoSelect = document.getElementById('correlativoSelect');
    const btnIntegrar = document.getElementById('btnIntegrarInvShopifyPPB');
    const btnVaciar = document.getElementById('btnVaciarInvShopifyPPB');
    const btnVerDatos = document.getElementById('btnVerDatosInvShopifyPPB');
    ...
    if (btnIntegrar) {
        btnIntegrar.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                html: `Se borrarán los datos actuales de 'Inventario Shopify PPB' y se procesarán los del lote <b>${selectedText}</b>.`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    Swal.fire({
                        title: 'Procesando...',
                        html: 'Migrando datos de Inventario Shopify PPB. Por favor espere.',
                        allowOutsideClick: false,
                        didOpen: () => { Swal.showLoading(); }
                    });

                    $.ajax({
                        url: '/ec_chans/app/integrador/inv_shopify_ppb/procesar_inv_shopify_ppb.php',
                        type: 'POST',
    ...
    if (btnVaciar) {
        btnVaciar.addEventListener('click', function() {
            Swal.fire({
                title: '¿Estás seguro?',
                text: "¡Todos los registros de Inventario Shopify PPB serán eliminados permanentemente!",
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    $.post('/ec_chans/app/integrador/inv_shopify_ppb/vaciar_inv_shopify_ppb.php', function(response) {
                        if(response.success) {
    ...
    if (btnVerDatos) {
        btnVerDatos.addEventListener('click', function() {
            ...
            $.post('/ec_chans/app/integrador/inv_shopify_ppb/obtener_datos_grid_inv_shopify_ppb.php', { correlativo: correlativo }, 'json')
