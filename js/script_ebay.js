...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const btnIntegrar = document.getElementById('btnIntegrarEbay');
    const btnVaciar = document.getElementById('btnVaciarEbay');
    const btnVerDatos = document.getElementById('btnVerDatosEbay');
    ...
    if (btnIntegrar) {
        btnIntegrar.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                html: `Se borrarán los datos actuales de 'eBay' y se procesarán los del lote <b>${selectedText}</b>.`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    Swal.fire({
                        title: 'Procesando...',
                        html: 'Migrando datos de eBay. Por favor espere.',
                        allowOutsideClick: false,
                        didOpen: () => { Swal.showLoading(); }
                    });

                    $.ajax({
                        url: '/ec_chans/app/integrador/ebay/procesar_ebay.php',
                        type: 'POST',
    ...
    if (btnVaciar) {
        btnVaciar.addEventListener('click', function() {
            Swal.fire({
                title: '¿Estás seguro?',
                text: "¡Todos los registros de eBay serán eliminados permanentemente!",
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    $.post('/ec_chans/app/integrador/ebay/vaciar_ebay.php', function(response) {
                        if(response.success) {
    ...
    if (btnVerDatos) {
        btnVerDatos.addEventListener('click', function() {
            ...
            $.post('/ec_chans/app/integrador/ebay/obtener_datos_grid_ebay.php', { correlativo: correlativo }, 'json')
