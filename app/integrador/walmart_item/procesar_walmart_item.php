<?php
// /app/integrador/walmart_item/procesar_walmart_item.php
...
if (isset($_POST['importacion_id'])) {
    ...
    try {
        ...
        if (!$import_data || empty($import_data['url_walmart_item'])) {
            throw new Exception("No se encontró un archivo 'Walmart Item' para esta importación.");
        }
        
        $ruta_relativa = $import_data['url_walmart_item'];
        ...
        vaciarTablaWalmartItem($link);
        
        $registros_procesados = procesarEInsertarWalmartItem($ruta_absoluta, $import_data, $link);
        
        if ($registros_procesados === 0) {
            throw new Exception("No se procesó ningún registro. Verifique el contenido del archivo.");
        }

        $description = "Se integraron $registros_procesados registros a 'tbl_walmart_item' desde la importación " . $import_data['fecha_correlativa_display'];
        registrarEvento("Integración de Walmart Item", "Integración", $description, $importacion_id, $link);
