<?php
// /app/integrador/ebay/procesar_ebay.php
...
if (isset($_POST['importacion_id'])) {
    ...
    try {
        ...
        if (!$import_data || empty($import_data['url_ebay'])) {
            throw new Exception("No se encontró un archivo 'eBay' para esta importación.");
        }
        
        $ruta_relativa = $import_data['url_ebay'];
        ...
        vaciarTablaEbay($link);
        
        $registros_procesados = procesarEInsertarEbay($ruta_absoluta, $import_data, $link);
        
        if ($registros_procesados === 0) {
            throw new Exception("No se procesó ningún registro. Verifique el contenido del archivo.");
        }

        $description = "Se integraron $registros_procesados registros a 'tbl_ebay' desde la importación " . $import_data['fecha_correlativa_display'];
        registrarEvento("Integración de eBay", "Integración", $description, $importacion_id, $link);
