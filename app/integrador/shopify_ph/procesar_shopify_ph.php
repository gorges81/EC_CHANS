<?php
// /app/integrador/shopify_ph/procesar_shopify_ph.php
...
if (isset($_POST['importacion_id'])) {
    ...
    try {
        ...
        if (!$import_data || empty($import_data['url_shopify_ph'])) {
            throw new Exception("No se encontró un archivo 'Shopify PH' para esta importación.");
        }
        
        $ruta_relativa = $import_data['url_shopify_ph'];
        ...
        vaciarTablaShopifyPH($link);
        
        // Llama a la función de procesamiento principal
        $registros_procesados = procesarEInsertarShopifyPH($ruta_absoluta, $import_data, $link);
        
        if ($registros_procesados === 0) {
            throw new Exception("No se procesó ningún registro. Verifique el contenido del archivo y el filtro de precios.");
        }

        $description = "Se integraron $registros_procesados registros a 'tbl_shopify_ph' desde la importación " . $import_data['fecha_correlativa_display'];
        registrarEvento("Integración de Shopify PH", "Integración", $description, $importacion_id, $link);
