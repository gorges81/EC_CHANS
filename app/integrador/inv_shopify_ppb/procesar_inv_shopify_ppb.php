<?php
// /app/integrador/inv_shopify_ppb/procesar_inv_shopify_ppb.php
...
if (isset($_POST['importacion_id'])) {
    ...
    try {
        ...
        if (!$import_data || empty($import_data['url_inv_shopify_ppb'])) {
            throw new Exception("No se encontró un archivo 'Inv Shopify PPB' para esta importación.");
        }
        
        $ruta_relativa = $import_data['url_inv_shopify_ppb'];
        ...
        vaciarTablaInvShopifyPPB($link);
        
        $registros_procesados = procesarEInsertarInvShopifyPPB($ruta_absoluta, $import_data, $link);
        
        if ($registros_procesados === 0) {
            throw new Exception("No se procesó ningún registro. Verifique el contenido del archivo.");
        }

        $description = "Se integraron $registros_procesados registros a 'tbl_inv_shopify_ppb' desde la importación " . $import_data['fecha_correlativa_display'];
        registrarEvento("Integración de Inv Shopify PPB", "Integración", $description, $importacion_id, $link);
